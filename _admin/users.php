...
<div class="newuser">
<h3 class="createtitle" style="color:#CA5;">
创建代理账户
</h3>
...
?>
<h2 style="color:#F00;">用户创建失败：<br />用户名‘<?php echo $user_name; ?>’已被占用</h2>
<?php
}
?>

<form action="" method="post">
<label>实名：</label> <input name="real_name" type="text" /><br />
<label>用户名：</label> <input name="user_name" type="text" value="B<?php echo strtotime('now'); ?>" /> <span style="font-size:10px; white-space:nowrap; color:#999;">英文字母、数字组合</span><br />
<label>密码：</label> <input name="pass_word" type="text" value="<?php echo genRandomString(); ?>" /> <span style="font-size:10px; white-space:nowrap; color:#999;">英文字母、数字组合</span><br />
<?php
if($account_level==0){
?>
...
<input type="hidden" name="given_by" value="" />
<?php
}else{
?>
<input type="hidden" name="account_level" value="2" />
<input type="hidden" name="given_by" value="<?php echo $username; ?>" />
<?php
}
?>
<p style="margin-top:0;">
<label style="display:inline-block; float:left;">备注信息：</label>
<textarea name="more_info" style="width:260px; height:70px; display:inline-block; float:left; margin-left:5px;"></textarea>
<br style="clear:both;" />
</p>

<p style="text-align:right; padding-right:20px; margin:0;">
<input type="submit" name="tianjia" style="background-color:#960; padding:3px 20px; font-size:18px; color:#FFF; border-style:solid; border-width:1px; cursor:pointer;" value="创建" />
</p>
</form>

</div>

<h3 class="listtitle" style="padding:0; margin:0 258px 15px 8px; border-bottom-style:solid; border-bottom-width:1px; border-color:#960;"><span style="display:inline-block; color:#960; background-color:#f1e7df; font-size:20px; margin:65px 0 0 0; padding: 3px 12px; border-style:solid; border-width:1px;
border-color:#960;">代理列表</span></h3>
...
if($account_level==0){	
	...
	?>
		<div class="userbox">
            <div class="level1user">
                ...
                <p class="realname"><?php echo $row['real_name']; ?></p>
                <p>用户名：<?php echo $row['user_name']; ?></p>
                <p>密码：<?php echo $row['pass_word']; ?></p>
                备注信息：<p class="tipsbox"><?php echo $row['more_info']; ?></p>
                <p><a href="/_admin/history.php?agent=<?php echo $row['user_name']; ?>">查看历史纪录</a></p>
            </div>
            ...
            <div class="level2user">
                ...
                <a class="l2realname" title="点击查看历史纪录" href="/_admin/history.php?agent=<?php echo $row_level2['user_name']; ?>"><?php echo $row_level2['real_name']; ?></a>
                </p>
                <?php
				}
				?>
	...
	foreach($conn->query($sql_user_ind) as $row_ind){
		...
		if($row_ind['account_level']==2){
			...
			?>
		<p class="realname"><?php echo $row_ind['real_name']; ?></p>
        <p>用户名：<?php echo $row_ind['user_name']; ?></p>
        <p>密码：<?php echo $row_ind['pass_word']; ?></p>
        备注信息：<p class="tipsbox"><?php echo $row_ind['more_info']; ?></p>
        <p><a href="/_admin/history.php?agent=<?php echo $row_ind['user_name']; ?>">查看历史纪录</a></p>
