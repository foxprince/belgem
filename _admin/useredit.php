...
if (isset ( $_POST ['user_name'] ) && isset ( $_POST ['pass_word'] )) {
	...
	if (! $existinguser) {
		$sql_renew = 'UPDATE users SET user_name = ?, pass_word = ?, real_name = ?, more_info = ?, account_level = ? ,disable = ? WHERE id = ?';
...
<hr />
  ...
  <div class="newuser">
    <h3 class="createtitle">编辑代理账户</h3>
...
<?php
	...
	?>
<h2 style="color: #F00;">
      用户修改失败：<br />用户名‘<?php echo $user_name; ?>’已被占用</h2>
<?php
}
...
<form action="" method="post">
      <label>实名：</label> <input name="real_name" type="text"
        value="<?php echo htmlentities($row['real_name'], ENT_COMPAT, 'UTF-8') ?>" /><br /> <label>用户名：</label> <input
        name="user_name" type="text" value="<?php echo htmlentities($row['user_name'], ENT_COMPAT, 'UTF-8') ?>" />
      英文字母、数字组合<br /> <label>密码：</label> <input name="pass_word" type="text"
        value="<?php echo htmlentities($row['pass_word'], ENT_COMPAT, 'UTF-8') ?>" /> 英文字母、数字组合<br />
<?php
if ($account_level == 0) {
	?>
...
<p style="margin-top: 0;">
        ...
        <textarea name="more_info"
          style="width: 260px; height: 70px; display: inline-block; float: left; margin-left: 5px;"><?php echo $row['more_info']; ?></textarea>
        <br style="clear: both;" /> <label>账户状态：</label> <select name="disable">
          <option value="1" <?php if($row['disable']==1){echo ' selected="selected"';} ?>>停用</option>
          <option value="0" <?php if($row['disable']==0){echo ' selected="selected"';} ?>>正常</option>
        </select>
      </p>
      <p style="text-align: right; padding-right: 20px; margin: 0;">
        <input type="submit" name="tianjia"
          style="background-color: #06F; padding: 3px 20px; font-size: 18px; color: #FFF; border-style: solid; border-width: 1px; cursor: pointer;"
          value="保存" />
      </p>
    </form>
  </div>
