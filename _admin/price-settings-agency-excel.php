...
logger('general_discount:'.$general_discount);
if(isset($_POST['discount_percentage'])){
	...
	if($thenew_disc_num==$general_discount){
		...
		$sql_disc_update='UPDATE price_discount SET excel_discount_agency = '.$thenew_disc_num;
...
function updateRule(ruleID){
	...
	if(idoftherule=='new'){
		$.post(
				"addRule.php", 
				{source:'excel', target:'agency', carat_from: $carat_from, carat_to: $carat_to, color: $color, clarity: $clarity, shape: $shape, cut: $cut, polish: $polish, symmetry: $symmetry, certificate: $certificate, fluo: $fluo, the_para_value:$the_para_value}, 
				function(data){
					if(data=='ok'){
		...
		$.post(
				"updateRule.php", 
				{source:'excel', target:'agency', recordid:idoftherule, carat_from: $carat_from, carat_to: $carat_to, color: $color, clarity: $clarity, shape: $shape, cut: $cut, polish: $polish, symmetry: $symmetry, certificate: $certificate, fluo: $fluo, the_para_value:$the_para_value}, 
				function(data){
					if(data=='ok'){
						$('button#modifyrules-'+idoftherule).html('保存成功！');
					}else{
...
function deleteRule(ruleID){
	...
	if(r){
		...
		$.post(
				"deleteRule.php", 
				{source:'excel', target:'agency', recordid:idoftherule}, 
				function(data){
					if(data=='ok'){
...
<?php
$sql_rules='SELECT * FROM price_settings_excel_agency ORDER BY priority ASC';
...
<a id="update_data_btn" href="import_excel.php">更新数据</a>
<p style="font-size:12px; padding-left:0px; margin-bottom:35px;"><span style="color:#F00;">注：</span>制定新规则以后要重新上传Excel文件才能生效</p>
</div>
