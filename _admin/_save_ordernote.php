...
if(isset($_POST['stock_ref']) && isset($_POST['content'])){
	$stock_ref=$_POST['stock_ref'];
	$content=$_POST['content'];
	$sql_UPDATE='UPDATE diamonds SET comment = "'.$content.'" WHERE stock_ref = "'.$stock_ref.'" ';
	...
	if($insertOK){
		...
		$the_action='操作：添加备注；操作人：'.$operator.'；操作对象：钻石编号'.$stock_ref;
