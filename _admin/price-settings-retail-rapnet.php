...
if(isset($_POST['discount_percentage'])){
	...
	if($thenew_disc_num==$general_discount){
		...
		$sql_disc_update='UPDATE price_discount SET rapnet_discount_retail = '.$thenew_disc_num;
...
function updateRule(ruleID){
	...
	if(idoftherule=='new'){
		$.post(
				"addRule.php", 
				{source:'rapnet', target:'retail', carat_from: $carat_from, carat_to: $carat_to, color: $color, clarity: $clarity, shape: $shape, cut: $cut, polish: $polish, symmetry: $symmetry, certificate: $certificate, fluo: $fluo, the_para_value:$the_para_value}, 
				function(data){
					if(data=='ok'){
		...
		$.post(
				"updateRule.php", 
				{source:'rapnet', target:'retail', recordid:idoftherule, carat_from: $carat_from, carat_to: $carat_to, color: $color, clarity: $clarity, shape: $shape, cut: $cut, polish: $polish, symmetry: $symmetry, certificate: $certificate, fluo: $fluo, the_para_value:$the_para_value}, 
				function(data){
					if(data=='ok'){
						$('button#modifyrules-'+idoftherule).html('保存成功！');
					}else{
...
function deleteRule(ruleID){
	...
	if(r){
		...
		$.post(
				"deleteRule.php", 
				{source:'rapnet', target:'retail', recordid:idoftherule}, 
				function(data){
					if(data=='ok'){
...
<h2>价格设定：零售网站价格 由Rapnet提取的数据</h2>
...
<?php
$sql_rules='SELECT * FROM price_settings_rapnet_retail ORDER BY priority ASC';
